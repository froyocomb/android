...
Java 5 is required for compiling Eclair. To install it, run the script again - select the 2nd option in the main menu and then select option 1. The script can also change the default Java version, which can be useful if compiling different Android versions.
...
To initialize a repository tree using one of the manifests provided by this project, execute a command like this (see the list of manifests above for available `<build>`s):
    ...
    repo init -u https://github.com/froyocomb/android.git -b what-dessert-is-eclair-anyway -m <build>.xml
